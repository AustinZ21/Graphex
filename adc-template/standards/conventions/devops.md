...
- **Automatic MCP Installation**: Project bootstrap SHOULD automatically install or refresh the paired `cga-mcp-server` profile in `.adc/cga-relay/mcp/mcp-servers.json` using environment-variable backed credentials.
...
- **CGA MCP Server Profile Baseline**: ADC templates MUST ship a `cga-mcp-server` entry in `.adc/cga-relay/mcp/mcp-servers.json` so downstream repositories auto-inherit CGA MCP Server wiring.
...
- **Failure Handling**: If CGA reporting or indexing fails, continue local build/test validation, record the failure in `.adc/cga-relay/scratchpad/session.md`, and retry when CGA is reachable.
